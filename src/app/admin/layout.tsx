...
const AdminSidebar = dynamic(
  () => import("@/components/common/admin-sidebar").then((m) => m.AdminSidebar),
  {
    ...
    loading: () => (
      <div
        className="hidden md:block fixed inset-y-0 left-0 w-[16rem] bg-[var(--ds-neutral-6)] border-r border-[var(--ds-primary-1)]"
        aria-hidden
      />
    ),
  }
      ...
      <SidebarInset className="bg-transparent relative">
        <div className="flex min-h-screen flex-col">
          <header className="flex h-12 shrink-0 items-center gap-2 px-3">
            <SidebarTrigger className="-ml-1 text-[var(--ds-primary-2)] hover:text-[var(--ds-primary-pure)]" />
          </header>
        ...
        {(loading || isAdmin === null) && (
          <div className="pointer-events-none absolute inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <Loader2 className="h-12 w-12 animate-spin text-[var(--ds-primary-1)]" />
          </div>
        )}
      </SidebarInset>
    </SidebarProvider>
